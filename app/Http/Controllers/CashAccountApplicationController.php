...
class CashAccountApplicationController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($request->has('form_request_token')) {
            $formRequest = \App\Models\ClientFormRequest::where('token', $request->form_request_token)->first();
            if ($formRequest && $formRequest->status === 'submitted') {
                return back()->withErrors('This form has already been submitted.');
            }
        }
        ...
        $rules = [
            ...
            'g-recaptcha-response' => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    if (!$secretKey) {
                        // If secret key is not set, skip validation (optional, depends on environment)
                        return;
                    }
        ...
        try {
            ...
            \App\Models\Frontuser::firstOrCreate(
                ['email' => $request->email],
                [
                    'name' => $request->contact_person,
                    'password' => $user->wasRecentlyCreated ? $password : $user->password,
                ]
            );
            ...
            $app = BusinessCreditApplication::create([
                ...
                'signed_client_name' => $request->sing_client_name,
            ...
            if ($request->has('form_request_token')) {
                \App\Models\ClientFormRequest::where('token', $request->form_request_token)
